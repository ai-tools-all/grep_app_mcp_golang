...
# Usage: ./build_cross_platform.sh [--compress]
...
GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o "$OUTPUT_DIR/${BINARY_NAME}_linux_arm64" .
...
if [ -n "$NDK_PATH" ]; then
    ...
    CGO_ENABLED=1 GOOS=android GOARCH=arm64 go build -ldflags="-s -w" -o "$OUTPUT_DIR/${BINARY_NAME}_android_arm64" .
    ...
    CGO_ENABLED=1 GOOS=android GOARCH=amd64 go build -ldflags="-s -w" -o "$OUTPUT_DIR/${BINARY_NAME}_android_amd64" .
    ...
    CGO_ENABLED=1 GOOS=android GOARCH=arm go build -ldflags="-s -w" -o "$OUTPUT_DIR/${BINARY_NAME}_android_arm" .
else
